 ...
 * @returns {*|Bluebird.Promise|Promise}
 */
MysqlDb.prototype.execute = function (sql,options) {
  'use strict';
  var self = this;
  if (!isEmpty(options.trans)) {
    return self.t_query(options.trans,sql).then(function (data) {
      if (isArray(data)) {
        return data
      }
      if (data.insertId) {
...
//进行事务语句
MysqlDb.prototype.t_query = function(t,sql){
  'use strict';
  return this.mysql.t_query(t,sql)
}
//提交事务
MysqlDb.prototype.t_commit = function(t){
  'use strict';
  return this.mysql.t_commit(t)
}
//事务回滚
MysqlDb.prototype.t_rollback = function(t){
  'use strict';
  return this.mysql.t_rollback(t)
}




//严重bug 如果大量链接 事务执行就不在同一个connect里面导致 有些事务直接卡死
// /**
//  * 启动事务
//  * @returns {*|Bluebird.Promise|Promise|*}
//  */
