...
let Db = require('./Lib/MysqlDb');
let defaultConfig = require('./Conf/config');
let util = require('util');
let querystring = require('querystring');
let Common = require('./Common/common');
let extend = Common.extend;
let getPromise = Common.getPromise;
let getObject = Common.getObject;
let isString = Common.isString;
let isNumber = Common.isNumber;
let isNumberString = Common.isNumberString;
let isArray = Common.isArray;
let isObject = Common.isObject;
let isBoolean = Common.isBoolean;
let isFunction = Common.isFunction;
let isScalar = Common.isScalar;
let isEmpty = Common.isEmpty;

let Mysql = function (config) {
...
Mysql.prototype.table = function (table) {
  'use strict';
  let tableName = this.tablePrefix + table;
...
Mysql.prototype.parseOptions = function (oriOpts, extraOptions) {
  ...
  if (isScalar(oriOpts)) {
    ...
    if (!isEmpty(oriOpts) && oriOpts.hasOwnProperty('trans') && !isEmpty(oriOpts.trans)) {
      trans = oriOpts.trans
      delete oriOpts.trans//暂时去除
    }
  ...
  //获取表名
  let table = options.table || self.tableName;
  //数据表别名
  if (options.alias) {
...
Mysql.prototype.parseType = function (data, key) {
  'use strict';
  let fieldType = this.fields._type[key] || '';
...
Mysql.prototype.thenAdd = function (data, where, returnType) {
  ...
  return this.where(where).find().then(function (findData) {
    if (!isEmpty(findData)) {
      let idValue = findData[self.getPk()];
...
Mysql.prototype.find = function (options) {
  ...
  return this.parseOptions(options, {limit: 1}).then(function (options) {
    parsedOptions = options;
    return self.db.select(parsedOptions);
  }).then(function (data) {
    return data[0] || {};
  });
};
...
Mysql.prototype.countSelect = function (options, pageFlag) {
  ...
  return this.parseOptions(options).then(function (options) {
    ...
    return self.options({
      ...
      alias: options.alias,
      table: options.table
    }).count((options.alias || options.tableName || self.tableName) + '.' + self.getPk());
  }).then(function (count) {
...
Mysql.prototype.commit = function(t){
  'use strict';
  let self = this;
  return self.db.t_commit(t)
}

Mysql.prototype.rollback = function(t){
  'use strict';
  let self = this;
  return self.db.t_rollback(t)
}
